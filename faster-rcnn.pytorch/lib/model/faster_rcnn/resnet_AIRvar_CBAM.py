...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False, use_RCNN_sigmoid=False, att_type=None):
    self.model_path = 'data/pretrained_model/resnet%d_caffe.pth' % num_layers
    self.dout_base_model = 1024
    self.pretrained = pretrained
    self.class_agnostic = class_agnostic
    self.num_layers = num_layers
    self.use_RCNN_sigmoid = use_RCNN_sigmoid
  ...
  def _init_modules(self):
    if self.num_layers == 50:
      ...
      raise AssertionError('resnet%d is not supported architecture!' % self.num_layers)
    ...
    if resnet.bam4 is not None:
      self.RCNN_top = nn.Sequential(resnet.layer4, resnet.bam4)
    else:
