...
class OIDv2(nn.Module):
    def __init__(self, baseFolder='models', use_detectorAIR23=False):
        self.path_to_InstModel = os.path.join(baseFolder, 'InstModel')

        self.use_detectorAIR23 = use_detectorAIR23
        self.use_AIR_detector = True
        ...
        if self.use_AIR_detector:
            ...
            self.detectorAIR = None

        if not self.use_COCO_detector and not self.use_AIR_detector:
            print('Both detectors are not activated. Please check it.')

        # classifier
        self.classifier = ModelManagerNN(self.path_to_InstModel, detection_model=self.detectorAIR, use_all_layers=True)    # det_model is used to calcuate att_map
    ...
    def detect(self, image):
        ...
        if self.use_AIR_detector:
            ret_bbox_score_class_AIR = self.detectorAIR.detect(image)
        else:
            ret_bbox_score_class_AIR = []

        if not self.use_COCO_detector and not self.use_AIR_detector:
            print('Both detectors are not activated. Please check it.')

        ret_bbox_score_class = ret_bbox_score_class_COCO5 + ret_bbox_score_class_AIR
        # crop, classify, replace, result

        ret_bbox_score_inst = self.classifier.classify(image, ret_bbox_score_class)
        # ret_bbox_score_inst = ret_bbox_score_class_COCO5 + ret_bbox_score_inst_AIR
    ...
    def register(self, image, category_name, instance_name):
        ...
        ret_bbox_score_class = ret_bbox_score_class_AIR + ret_bbox_score_class_COCO
...
class ModelManagerNN():
    def __init__(self, instBase_path, num_hid_ftr=2048, fix_base_net=True, num_layer=1, fc_type='linear',
                 force_to_train=False, detection_model=None, use_all_layers=False):
        self.modelList = []
        # self.modelListGMM = []
        # self.modelListkNN = []
    ...
    def load_DB(self, path, category_index=None, verbose=True):
        ...
        for iC, objCate in enumerate(listCategory):        # object category
            ...
            nnC = NNClassifier(objCate,
                               ...
                               use_all_layers=self.use_all_layers)  # construct classifier
    ...
    def get_gradMap(self, image_crop_bgr_224, c_name, vis=False):
        ...
        for item in bbox_score_class:
            ...
            if c_name in self.modelCategorynameList:   # classifier
                ...
                if self.margin_ratio == 0:
                    image_crop_for_attention = image[y1:y2, x1:x2, :]
                    image_crop_for_clf = image[y1:y2, x1:x2, :]
                else:
                    crop_w_half = int((x2 - x1) * self.margin_ratio)  # add 25% to left and 25% to right
                    crop_h_half = int((y2 - y1) * self.margin_ratio)

                    crop_x1 = max(x1 - crop_w_half, 0)
                    crop_x2 = min(x2 + crop_w_half, image.shape[1])

                    crop_y1 = max(y1 - crop_h_half, 0)
                    crop_y2 = min(y2 + crop_h_half, image.shape[0])
                ...
                gradMap = self.get_gradMap(image_crop_bgr_224, c_name, vis=True)
    ...
    def save_DB(self, image, bbox, category_name, instance_name):
        # crop
        if self.margin_ratio == 0:
            crop_image = image[bbox[1]:bbox[3], bbox[0]:bbox[2], :]
        else:
            crop_w_half = int((bbox[2] - bbox[0]) * self.margin_ratio)  # add 25% to left and 25% to right
            crop_h_half = int((bbox[3] - bbox[1]) * self.margin_ratio)

            crop_x1 = max(bbox[0] - crop_w_half, 0)
            crop_x2 = min(bbox[2] + crop_w_half, image.shape[1])

            crop_y1 = max(bbox[1] - crop_h_half, 0)
            crop_y2 = min(bbox[3] + crop_h_half, image.shape[0])
        ...
        cv2.imwrite(pathSaveTimestamp + '.png', crop_image)
